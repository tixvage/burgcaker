...
#define array_append(a, d, l) \
    do { \
        ...
        if (a.data == NULL) { \
            a.data = a.allocator.alloc((a.capacity) * sizeof(a.data[0])); \
            for (int __i = 0; __i < __len; __i++) { a.data[__i] = d[__i]; } \
            a.len += __len; \
        } else { \
            a.data = a.allocator.realloc(a.data, a.capacity * sizeof(a.data[0])); \
...
#define array_free(a) do { a.allocator.free(a.data); a.data = NULL; a.len = 0; a.capacity = 0; } while(0)

#define array_copy(a, T) ({ T ___na = a; ___na.data = ___na.allocator.alloc((___na.capacity) * sizeof(___na.data[0])); memcpy(___na.data, a.data, sizeof(___na.data[0]) * ___na.len); ___na; })
